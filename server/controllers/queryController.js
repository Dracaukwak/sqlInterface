...
async function executeQuery(req, res, next) {
  const { query, offset = 0, limit = 10, skipPagination = false } = req.body;
  ...
  try {
    ...
    if (query.trim().toLowerCase().startsWith('select') && !skipPagination) {
      ...
      if (hasLimit) {
        ...
        rows = await databaseService.executeQuery(cleanQuery);
        ...
        rows = await databaseService.executeQuery(paginatedQuery);
        actualLimit = limit;
      }
    ...
    // For SELECT queries with skipPagination flag
      ...
      const rows = await databaseService.executeQuery(query.trim().replace(/;+$/, ''));
